...
- **`Config.Sink` is a terminal handler, not a pipeline bypass** — A caller-supplied `Sink` receives fully decorated records: the dedup stage, `FilterRules`, `GlobalFields`, `AddSource` attribution, `Sampling`, `MetricsHandler`, `BufferSize`, `Hook` and `SetLevel` all apply on top of it, exactly as they do on the built-in Text/JSON handler. Because the dedup stage materializes `With` attributes into the record, a `Sink` never receives `WithAttrs`/`WithGroup` calls (issue #42). `Config.Handler` is a deprecated alias for `Sink` (`Sink` wins if both are set) and behaves identically (issue #11).
...
- **Every key appears once per line, and global fields come first** — `DedupHandler` is the innermost stage and must stay directly above the terminal handler: slog's Text/JSON handlers pre-format `WithAttrs`, and a pre-formatted attr cannot be deduplicated. It holds every `WithAttrs`/`WithGroup`, then resolves each key to the value of its last occurrence at the position of its first; `GlobalFields` are its pinned attrs (first, sorted, never overridden, not even by `With`); `time`/`level`/`msg` are reserved and a colliding attr becomes `attr.<key>`; same-key groups merge (issue #42). `GlobalFieldsHandler` is no longer in the pipeline and remains only for hand-built chains.
...
- **Source attribution is opt-in, lives under `source`, and comes from `slog.Record.PC`, never from a live stack walk** — `SourceHandler` runs only when `Config.AddSource` is set and emits `source{function,file,line}`, the key slog reserves for this, leaving `component` to the caller. It resolves the call site from the PC slog captured at the log call, so attribution survives asynchronous delivery through the buffer (issues #7, #42). `ComponentHandler` is deprecated and out of the pipeline.
...
| Every key appears once per line | With `AddSource` on, a caller attribute named `source` is replaced by the pipeline's own `source` group | `SourceHandler` sits above the dedup stage and delivers its group as a record attr, so it goes through the same last-wins rule as any other key; `source` is slog's reserved name for exactly this | #42 |
| Pre-formatting at the sink | A `Sink` never receives `WithAttrs`/`WithGroup`; every attribute arrives on the record, nested with `slog.GroupValue` | The dedup stage must see every attribute after all decorators have run, and a pre-formatted attribute cannot be compared against anything | #42 |
